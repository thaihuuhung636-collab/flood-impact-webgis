...
function buildUI(){let flood=document.getElementById('floodLayers'),base=document.getElementById('baseLayers');cfg.layers.forEach(l=>{addFloodLayer(l);flood.insertAdjacentHTML('beforeend',cardHTML(l.id,l.caption||l.alias,l.item+' · '+l.frames.length+' frame','flood'));});(cfg.baseLayers||[]).forEach(b=>{addBaseLayer(b);base.insertAdjacentHTML('beforeend',cardHTML(b.id,b.name,(b.group?b.group+' · ':'')+b.type,'base'));});document.querySelectorAll('input[type=checkbox][data-id]').forEach(cb=>cb.onchange=e=>{let id=e.target.dataset.id,group=e.target.dataset.group;let vis=e.target.checked;if(group==='flood'){if(vis)state.active.add(id);else state.active.delete(id);}if(state.layers[id])state.layers[id].setVisible(vis);if(state.hitLayers[id])state.hitLayers[id].setVisible(vis);refresh(true);});document.getElementById('layerSearch').oninput=e=>{let q=e.target.value.toLowerCase().trim();document.querySelectorAll('.card').forEach(c=>c.classList.toggle('hidden',q && !c.dataset.name.includes(q)));};(cfg.baseLayers||[]).forEach(b=>{if(b.visible){let cb=document.querySelector(`input[data-id="${b.id}"]`);if(cb){cb.checked=true;cb.onchange({target:cb});}}});if(cfg.layers[0]){let first=document.querySelector(`input[data-id="${cfg.layers[0].id}"]`);if(first){first.checked=true;first.onchange({target:first});}}else refresh(true);}
